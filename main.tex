...
We use the convention for the Frenet-Serret equations
\begin{equation}
\label{eq:fs}
\partial_s \tang = - \kappa \nor, \quad \partial_s \nor = \kappa \tang.
\end{equation}
...
\begin{enumerate}
\item \([\partial_s, \partial_t] = \kappa^2 \partial_s\)
...
Z'' &\leq \frac{(-1)^{\delta}\sqrt{1-\cos^2\theta}}{4} (\kappa_{p_0} + (-1)^{\xi+1}\kappa_{q_0}) + (1-\xi) \frac{1-\cos^2\theta}{d(p_0,q_0)}.
\end{align*}
...
\[
\partial_x (\ell\circ\alpha)^{-1} = -\frac{1}{2}.
\]
...
-1
\end{pmatrix} \\
&= \frac{\sqrt{1-\cos^2\theta}}{4} (-1)^{\delta} (\kappa_{p_0} + (-1)^{\xi + 1} \kappa_{q_0})
\end{align*}
...
\[
Z'' \leq - \frac{\sqrt{1-\cos^2 \theta}}{4} (\kappa_{p_0} + \kappa_{q_0}) \leq 0
\]
...
\partial_t \Phi \leq 0
\]
at $(p_0, q_0, t_0)$. That is, using the time variation formula from \Cref{lem:distance_evolution}, at \((p_0, q_0, t_0)\),
\[
...
\]
Combining this with equation \eqref{eq:time_ineq} and noting that \Cref{prop:barrier} gives \(Z' = \cos\theta\) completes the proof.
\end{proof}
