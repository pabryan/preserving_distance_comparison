...
The term \(h\) is a combination of local and global terms. That is,
\[
h : C^{\infty} (\S^1 \to \R^2) \to C^{\infty} (\S^1 \to \R^2)
\]
depends on \(X\) and it's derivatives along with global terms which we may think of as functionals \(C^{\infty} (\S^1 \to \R^2) \to \R\). For geometric quantities, \(h\) depends on \(\kappa\) and typically integrals of the form \(\int_{\S^1} f(\kappa) ds\), but may also involve geometrically invariant integrals of the support function \(\sfn = \inpr{X}{\nor}\) such as in the total enclosed area \(A = \tfrac{1}{2} \int_{\S^1} \sfn ds\). We may also allow \(h\) to be \emph{anisotropic} so that it depends pointwise on \(X\) and \(\nor\). The support function is such a quantity.

In particular, the \emph{curve shortening flow} is given by
\begin{equation}
\label{eq:csf}
\tag{CSF}
h \equiv 0,
\end{equation}
\emph{length preserving curve shortening flow} is given by
\begin{equation}
\label{eq:lpcsf}
\tag{LPCSF}
h = \frac{1}{2\pi} \int_{\S^1} \kappa^2 ds,
\end{equation}
while the \emph{enclosed area preserving curve shortening flow} is given by
\begin{equation}
\label{eq:apcsf}
\tag{APCSF}
...
\item \(\partial_t \tang = -\partial_s (\kappa-h) \nor\)
\item \(\partial_t \nor = \partial_s (\kappa-h) \tang\)
\item \(\partial_t \kappa = \partial_s^2 (\kappa-h) + \kappa^2(\kappa - h)\).
\end{enumerate}
\end{lemma}
...
\begin{align*}
\partial_t d_t(p, q) &= -(\kappa_q-h_q) \inpr{\nor_q}{w} + (\kappa_p-h_p) \inpr{\nor_p}{w} \\
...
\[
\begin{split}
\partial_t \inpr{\gamma(q) - \gamma(p)}{\gamma(q) - \gamma(p)} &= 2 \inpr{-(\kappa(q)-h(q)) \nor(q)}{\gamma(q) - \gamma(p)} \\
&\quad  - 2 \inpr{-(\kappa(p)-h(p)) \nor(p)}{\gamma(q) - \gamma(p)} \\
&= 2 d\left[-\inpr{(\kappa(q)-h(q))\nor(q)}{w} + \inpr{(\kappa(p)-h(p))\nor(p)}{w}\right]
\end{split}
\]
...
\[
\begin{split}
\partial_{\tau} Z & \geq 4 Z'' + 4Z'' - 4\xi \frac{1-(Z')^2}{Z} \\
&\quad + \left(\frac{L}{2\pi}\right)^2 \avg{\kappa(\kappa-h)} (Z - x Z') + \frac{L}{2\pi} Z' \int_p^q \kappa(\kappa-h) ds \\
&\quad + (-1)^{\delta} \frac{L}{2\pi} \sqrt{1-(Z')^2} [(-1)^{\xi}h_{q_0} + h_{p_0}]
\end{split}
\]
\end{thm}
...
\[
\begin{split}
\partial_t \left[\frac{2\pi}{L} d\right] - \varphi' \partial_t \left[\frac{2\pi}{L}\ell\right] &= \frac{2\pi}{L}\left[-(\kappa_{q_0}-h_{q_0}) \inpr{\nor_q}{w} + (\kappa_{p_0}-h_{p_0}) \inpr{\nor_p}{w}\right] + \frac{2\pi d}{L} \frac{1}{L} \int_{\gamma} \kappa(\kappa-h) ds \\
&\quad + \frac{2\pi}{L} \varphi' \int_p^q \kappa(\kappa-h) ds - \frac{2\pi\ell}{L} \frac{\varphi'}{L} \int_{\gamma}\kappa(\kappa-h) ds \\
&= \frac{2\pi}{L}\left[-(\kappa_{q_0}-h_{q_0}) \inpr{\nor_q}{w} + (\kappa_{p_0}-h_{p_0}) \inpr{\nor_p}{w}\right] \\
...
\label{eq:time_ineq}
\begin{split}
\partial_{\tau} \varphi &\geq \frac{L}{2\pi} (-1)^{\delta+1} \sqrt{1-(\varphi^2)} [(-1)^{\xi}\kappa_{q_0} + \kappa_{p_0} - ((-1)^{\xi}h_{q_0} + h_{p_0})] \\
...
\begin{split}
\partial_{\tau} \varphi &\geq 4\varphi'' - 4\xi \frac{1-(\varphi')^2}{\varphi} \\
&\quad + \left(\frac{L}{2\pi}\right)^2 \avg{\kappa(\kappa-h)} (\varphi - x \varphi') + \frac{L}{2\pi} \varphi' \int_p^q \kappa(\kappa-h) ds \\
&\quad + (-1)^{\delta} \frac{L}{2\pi} \sqrt{1-(\varphi')^2} [(-1)^{\xi}h_{q_0} + h_{p_0}].
\end{split}
\end{equation}
