...
\begin{rem}
In particular, if $\gamma$ is strictly convex, then already \(T=\tang_{p_0} \ne \tang_{q_0}\) whenever \(p_0 \ne q_0\). The lemma then says that the in the presence of a concave lower barrier, we obtain the same conclusion and we see this \emph{local concavity} of the profile gives convex like behaviour of the corresponding point. \Cref{prop:barrier} below gives a converse where convexity of the curve implies concavity of the profile. Thus we see a close relationship between convexity of the curve and concavity of the profile (even in a local sense).
\end{rem}
...
\begin{proof}
Fix $x_0 \in (0,2\pi)$. Let $Z(x)$ be a smooth function defined in neighbourhood of $x_0$ and such that $Z(x_0) = \chordarcprofile(x_0)$ and $Z(x) \leq \chordarcprofile(x)$. As in \Cref{prop:barrier}, choose $(p_0,q_0)$ with $p_0\ne q_0$ so that $\ell(p_0, q_0) = \tfrac{L}{2\pi} x_0$ and $\chordarcprofile(x_0) = \tfrac{2\pi}{L}d(p_0, q_0)$.  Then we have
\[
...
\]
The proof is completed by combining with equation \eqref{eq:time_ineq} while noting the change of \((-1)^{\delta}\) in equation \eqref{eq:time_ineq} to \((-1)^{\delta + 1} = -(-1)^{\delta}\) and recalling that \Cref{prop:barrier} gives \(Z' = \cos\theta\).
\end{proof}
...
\]
Thus we have \(\chordarcprofile(x, \tau) > \varphi_{\epsilon}(x, \tau)\) for \((x, \tau) \in (0, \bar{x}) \times (0, \bar{\tau})\) and by symmetry also for for \((x, \tau) \in (2\pi-\bar{x}, 2\pi) \times (0, \bar{\tau})\). The important point here is that these inequalities hold true \emph{for any} \(\lambda > 0\) with \(\bar{x}\) and \(\bar{\tau}\) independent of \(\lambda\).
...
\[
\partial_{\tau} \varphi_{\epsilon} - 4 \varphi_{\epsilon}'' - \left(\frac{L}{2\pi}\right)^2 (\varphi_{\epsilon} - x \varphi_{\epsilon}') \avg{\kappa^2} - \varphi_{\epsilon}' \frac{L}{2\pi} \int_{p_0}^{q_0} \kappa^2 ds \geq 0.
\]
...
\[
(\partial_{\tau} - \mathcal{L}) \varphi_{\epsilon}|_{(x_0, t_0)} \leq -\epsilon^2 e^{-\lambda \tau_0} \left(\lambda x_0(2\pi - x_0) + \mathcal{L} [x(2\pi-x)]|_{x_0}\right).
\]
From the argument above we know that \((x_0, \tau_0) \in [\bar{x}, 2\pi - \bar{x}] \times [0, \bar{\tau}]\) where \(x(x-2\pi) \geq \bar{x}(2\pi - \bar{x}) > 0\) has a positive lower bounded. Choose
\[
\lambda > \frac{\sup_{x \in [\bar{x}, 2\pi-\bar{x}]} \abs{\mathcal{L} [x(2\pi-x)]}}{\bar{x}(2\pi - \bar{x})}.
\]
Then \(\lambda\) depends only on \(\bar{x}\) (which only depends on \(\bar{\tau}\)) and is in particular independent of \((x_0, \tau_0)\) and
\[
